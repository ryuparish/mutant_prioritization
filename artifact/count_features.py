...
# Expects the dataframe BEFORE transforming with DataFrameMapper
def get_expanded_counts(dataframe,debug=False):
    """
    Returns feature and the length of the one-hot encoding after transforming
    DataframeMapper by counting the unique values of the columns that are of type "object".

    :param dataframe: A dataframe
    :param debug: Debugging flag
    :type dataframe: Dataframe
    :type debug: bool
    :returns: A dict mapping column name to number of unique values (1 if scalar)
    :type returns: dict
    """
    total_item_count = 0
    columnNameToCount = {}
    ...
    for idx, col in enumerate(dataframe.columns):
        if col in SELECTED_FEATURES:
            if dataframe[col].dtype == "object":
                ...
                columnNameToCount[col] = len(dataframe[col].unique())
            else: 
                if debug:
                    print(f"[{idx}, {col}] Count: 1 dtype: {dataframe[col].dtype}")
                columnNameToCount[col] = 1
                total_item_count += 1
